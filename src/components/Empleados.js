...
function EnhancedTableToolbar(props) {
  ...
  // const empleadosIndex = rows.findIndex((empleado) => empleado.em_id === selected);
  ...
  return (
    ...
    >
      ...
      <IconButton
        title="selecciona empleado para editar"
        onClick={() =>
          selected > 0
            ? handleShowEdit(
                rows[
                  rows.findIndex((empleado) => empleado.em_id === selected[0])
                ].em_nombre,
                rows[
                  rows.findIndex((empleado) => empleado.em_id === selected[0])
                ].em_cedula,
...
export default function Empleados() {
  ...
  const validar = () => {
    if (
      form.em_nombre.trimStart() === "" ||
      // form.em_cedula.trimStart() === "" ||
      form.em_celular.trimStart() === ""
    ) {
      console.log("prueba");
      setSnack(true);
    } else if (btn === "añadir") {
  ...
  //-----------------------------------------------
  const handleShowEdit = (em_nombre, em_cedula, em_celular) => {
    console.log(selected);
    handleOpen(true);
  ...
  const getEmpleados = async () => {
    const response = await axios.get(`${webservices}/empleados`);
  ...
  const agregarProducto = async () => {
    ...
    formData.append("em_nombre", form.em_nombre);
    formData.append("em_cedula", form.em_cedula);
    formData.append("em_celular", form.em_celular);

    if (file) {
      formData.append("em_photo", file);
    } else {
      formData.append("em_photo", null);
    }

    try {
      const response = await axios.post(`${webservices}/empleados`, formData);
  ...
  const actualizarProducto = async () => {
    ...
    try {
      const response = await axios.put(
        `${webservices}/empleados/${selected}`,
        formData
      );
      if (response && response.data) {
        setloading(true);
        setRows(rows.fill(response.data, form.em_id - 1, form.em_id));
        handleClose(false);
        setSelected([]);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = rows.map((n) => n.em_id);
  ...
  const handleClick = (event, em_id) => {
    const selectedIndex = selected.indexOf(em_id);
    let newSelected = [];

    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, em_id);
  ...
  const isSelected = (em_id) => selected.indexOf(em_id) !== -1;
        ...
        >
          <Box sx={style}>
            ...
            <CustomInput
              ...
              value={form.em_nombre}
              onChange={(e) => setForm({ ...form, em_nombre: e.target.value })}
            />
            <p id="unstyled-modal-description">Cedula</p>
            <CustomInput
              aria-label="Demo input"
              placeholder="Type something…"
              value={form.em_cedula}
              onChange={(e) => setForm({ ...form, em_cedula: e.target.value })}
            />
            <p id="unstyled-modal-description">Celular</p>
            <CustomInput
              aria-label="Demo input"
              placeholder="Type something…"
              value={form.em_celular}
              onChange={(e) => setForm({ ...form, em_celular: e.target.value })}
            />
              ...
              {visibleRows
                ? visibleRows.map((row, index) => {
                    ...
                    return (
                      <TableRow
                        hover
                        onClick={(event) => handleClick(event, row.em_id)}
                        role="checkbox"
                      ...
                      >
                        ...
                        <TableCell align="right">{row.em_nombre}</TableCell>
                        <TableCell align="right">{row.em_cedula}</TableCell>
                        <TableCell align="right">{row.em_celular}</TableCell>
